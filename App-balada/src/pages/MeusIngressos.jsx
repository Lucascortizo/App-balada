...
import { Ticket, Send, User, Users, Plus, Trash2, CheckCircle2, Link, Star, XCircle, Clock, X } from 'lucide-react';
...
export default function MeusIngressos() {
  ...
  // ================= LÓGICA DE EVENTOS ATIVOS VS HISTÓRICO =================
  // Coleta todos os IDs únicos em que o usuário está envolvido
  ...
  // Filtra Fantasmas e define quais eventos vão para o histórico
  const eventosProcessados = IDsTodosEventos.map(eventoId => {
    const festa = eventosGlobais.find(e => e.id === eventoId);
    if (!festa) return null; // Ignora ingressos de festas que o Admin já apagou

    // Regra do Histórico: Se a data do evento já passou há mais de 24h
    const eventoPassou = (Date.now() - new Date(festa.data).getTime()) > (24 * 60 * 60 * 1000);

    // Também consideramos como histórico se TODOS os ingressos/acessos da pessoa nesse evento estiverem como 'saiu' (finalizado)
    const meusIngressosNaFesta = ingressos.filter(i => i.eventoId === eventoId);
    const todosUsados = meusIngressosNaFesta.length > 0 && meusIngressosNaFesta.every(i => i.status === 'saiu' || i.status === 'encerrado' || i.status === 'usado');
    const meusVipNaFesta = reservas.filter(r => r.eventoId === eventoId);
    const todosCheckin = meusVipNaFesta.length > 0 && meusVipNaFesta.every(r => r.checkinFeito);

    const isHistorico = eventoPassou || (meusIngressosNaFesta.length > 0 && todosUsados) || (meusVipNaFesta.length > 0 && todosCheckin);

    return { eventoId, festa, isHistorico };
  }).filter(e => e !== null);
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] text-zinc-900 pb-32">
      ...
      <main className="px-6 max-w-md mx-auto space-y-6">
        ...
        {/* CAIXA DE ENTRADA (CONVITES) - SÓ MOSTRA NA ABA ATIVOS */}
            ...
            <div className="space-y-3">
              ...
              {espacosRecebidos.map(r => (
                <div key={r.id} className="bg-white/10 p-4 rounded-2xl border border-indigo-400/30">
                  <p className="font-bold text-sm mb-3 text-indigo-100">Titularidade VIP (<b className="text-white">{r.sigla}</b>) recebida de <br/><b className="text-white">{r.donoNome}</b></p>
                  <div className="flex gap-2">
        ...
        {eventosExibidos.length === 0 ? (
          ...
          /* ================= LISTAGEM DOS INGRESSOS E CAMAROTES ================= */
          eventosExibidos.map(dadosEvento => {
            const { eventoId, festa, isHistorico } = dadosEvento;
            ...
            const convitesDestaFesta = espacosComoConvidado.filter(c => c.eventoId === eventoId);
            ...
            return (
              <div key={eventoId} className={`rounded-[2.5rem] border shadow-sm overflow-hidden flex flex-col transition-all ${isHistorico ? 'bg-zinc-50 opacity-90 grayscale-[0.3]' : 'bg-white'}`}>
                ...
                {/* Header do Evento (Semelhante à Comanda) */}
                <div className={`p-6 text-white relative ${isHistorico ? 'bg-zinc-700' : 'bg-zinc-900'}`}>
                  ...
                  <p className="text-xs text-indigo-400 font-bold uppercase tracking-widest mt-1 flex items-center gap-1.5">
                    <Ticket className="w-3.5 h-3.5" /> Controle de Acesso
                  </p>
                </div>
                
                <div className="p-6 space-y-4">
                  {/* INGRESSOS PISTA */}
                  {ingressosDestaFesta.map((i, idx) => {
                    ...
                    return (
                      <div key={i.id} className="py-3 border-b border-zinc-100 last:border-0">
                        <div className="flex justify-between items-center mb-3">
                          <div>
                            <p className="font-black !text-zinc-900 flex items-center gap-1.5">
                              Pista <span className="text-[10px] bg-zinc-100 text-zinc-500 px-1.5 py-0.5 rounded-md">#{idx + 1}</span>
                            </p>
                          </div>
                          <button 
                            onClick={() => setTicketModal({ tipo: 'ingresso', id: i.id, status: i.status, nome: festa?.nome })} 
                            disabled={i.transferencia?.status === 'pendente' || isHistorico} 
                            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${jaUsou || isHistorico ? 'bg-zinc-100 text-zinc-400' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'}`}
                          >
                        ...
                        {!isHistorico && !i.transferencia && !jaUsou && (
                          <button 
                            onClick={() => setModalTransferencia({ aberto: true, id: i.id, tipo: 'ingresso' })} 
                            className="text-[10px] uppercase font-bold text-zinc-400 hover:text-indigo-600 transition flex items-center gap-1 w-fit mt-1"
                          >
                    ...
                    return (
                      <div key={r.id} className="py-3 border-b border-zinc-100 last:border-0">
                        <div className="flex justify-between items-center mb-3">
                          ...
                          <button 
                            onClick={() => setTicketModal({ tipo: 'espaco', id: r.id, status: jaEntrou ? 'usado' : 'valido', nome: festa?.nome })} 
                            disabled={r.transferencia?.status === 'pendente' || isHistorico} 
                            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${jaEntrou || isHistorico ? 'bg-zinc-100 text-zinc-400' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'}`}
                          >
                        ...
                        {!isHistorico && !jaEntrou && !r.transferencia && (
                          <div className="flex gap-4 mt-2">
                            <button onClick={() => setModalConvidados({ aberto: true, espaco: r })} className="text-[10px] uppercase font-bold text-indigo-600 flex items-center gap-1 bg-indigo-50 px-2 py-1 rounded-md hover:bg-indigo-100 transition">
                              <Users className="w-3 h-3" /> Gerenciar Lista VIP
                            </button>
                            <button onClick={() => setModalTransferencia({ aberto: true, id: r.id, tipo: 'espaco' })} className="text-[10px] uppercase font-bold text-zinc-400 hover:text-zinc-600 flex items-center gap-1 transition">
                  ...
                  {convitesDestaFesta.map((c) => (
                    <div key={c.id} className="py-3 border-b border-zinc-100 last:border-0 flex justify-between items-center">
                      ...
                      <button 
                        onClick={() => setTicketModal({ tipo: 'convidado', id: c.id, status: 'valido', nome: festa?.nome })} 
                        disabled={isHistorico}
                        className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${isHistorico ? 'bg-zinc-100 text-zinc-400' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'}`}
                      >
                        Mostrar QR
                      </button>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm p-6 animate-fade-in">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
            ...
            <div className="p-6 bg-zinc-50 border-b border-zinc-200 space-y-3">
              <button onClick={() => gerarLinkConvite(modalConvidados.espaco.id)} className="w-full bg-white border border-zinc-200 text-zinc-700 py-3.5 rounded-2xl flex items-center justify-center gap-2 text-sm font-black shadow-sm active:scale-95 transition">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => adicionarConvidado(modalConvidados.espaco)} disabled={isProcessando} className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 rounded-xl font-bold flex items-center justify-center transition disabled:opacity-50">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              <div className="flex justify-between items-center bg-indigo-50 p-3 rounded-2xl border border-indigo-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-indigo-600 font-black tracking-widest uppercase">Titular</p>
                  </div>
                </div>
              ...
              {modalConvidados.espaco.convidados?.map((c, idx) => (
                <div key={idx} className="flex justify-between items-center bg-white p-3 rounded-2xl border border-zinc-200 shadow-sm">
                  ...
                  <button onClick={() => removerConvidado(modalConvidados.espaco, idx)} className="text-red-500 bg-red-50 p-2.5 rounded-xl hover:bg-red-500 hover:text-white transition">
